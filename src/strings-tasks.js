...
function removeFirstOccurrences(str, value) {
  ...
  if (index !== -1) {
    ...
    const secondPart = str.slice(index + value.length);
...
function reverseWords(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    newStr = newStr.concat(words[i].split('').reverse().join(''));
    if (i !== words.length - 1) {
...
function extractNameFromTemplate(value) {
  if (value.startsWith('Hello, ') && value.endsWith('!')) {
    const words = value.replace(/[^a-zA-Z\s]/g, '').split(' ');
    if (words.length > 2) {
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (
      lowerAlphabet.includes(str.at(i)) ||
      upperAlphabet.includes(str.at(i))
    ) {
      newIndex = lowerAlphabet.indexOf(str.at(i).toLowerCase()) + 13;
      if (newIndex > lowerAlphabet.length - 1) {
        newIndex -= lowerAlphabet.length;
      }
      if (str.at(i) === str.at(i).toLowerCase()) {
        chars.push(lowerAlphabet.at(newIndex));
      } else {
        chars.push(upperAlphabet.at(newIndex));
      }
    } else {
      chars.push(str.at(i));
    }
